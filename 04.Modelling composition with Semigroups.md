...
준그룹은 두 개 이상의 값을 결합하는 방법입니다.

준집합은 **대수학**으로, 일반적으로 다음과 같은 특정 조합으로 정의됩니다:

- 하나 이상의 집합
- 해당 집합에 대한 하나 이상의 연산
- 이전 연산에 대한 0개 이상의 법칙

대수는 수학자가 불필요한 모든 것을 제거하여 가장 순수한 형태로 아이디어를 포착하는 방법입니다.

> 대수를 수정할 때 허용되는 연산은 대수 자체의 법칙에 따라 대수 자체에서 정의한 연산뿐입니다.

대수는 **인터페이스**의 추상화로 생각할 수 있습니다:

> 인터페이스가 수정될 때 허용되는 유일한 연산은 자체 법칙에 따라 인터페이스 자체에 의해 정의된 연산입니다.
